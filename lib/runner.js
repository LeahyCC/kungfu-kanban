...
// forward local <default> from origin (ref-update only — never touches the
// checkout) right before launch closes that window. No-ops quietly when
// offline (timeout) or when local isn't a plain repo; logs one line only
// when it actually moved the ref or when local has diverged (non-ff).
async function syncDefaultBranch(task) {
  ...
  } else if (/non-fast-forward|fetch first|rejected/i.test(fetch.err)) {
    appendTranscript(task.id, {
      kind: 'init',
...
// Next launchable card: queued, all deps done, highest priority first, then
...
function notifyBlocker(dep) {
  if (dep.status !== 'review' && dep.status !== 'backlog') return;
  ...
  const action = dep.status === 'backlog'
    ? 'queue or run it'
    : dep.error
...
function pumpQueue() {
  ...
  while (!cooldown.active() && slotsFree()) {
