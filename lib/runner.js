...
const { state, save, getTask, appendTranscript, flushTranscripts, clearTranscript, touch } = require('./store');
...
const lastAssistantText = new Map(); // taskId -> most recent streamed assistant text block
let onFinish = () => {};
...
// Frontend cards kept passing review on static code review alone — no agent
// ever proved the DOM rendered or a click handler fired. Every card gets this
// standing convention; it self-gates, so non-UI work just ignores it. On-demand
// npx/--no-save on purpose: playwright is deliberately not a dependency of this
// app (npx's cache and the browser cache are per-machine, so downloads happen
// once, not per card — see CLAUDE.md's PR protocol for why not postinstall).
// Note `npx --package playwright node script.js` does NOT work: modern npm
// puts the package's bin on PATH but sets no NODE_PATH for a child node.
const UI_VERIFY =
  'Standing convention — verifying UI work: if your changes touch UI (HTML/CSS/frontend JS), do not ' +
  'report done on code review alone. Prove the change in a real headless browser and state what you ' +
  'verified in your final report. Quick visual proof from any directory: ' +
  '`npx playwright screenshot <url> out.png`. Scripted checks (clicks, console errors, computed ' +
  'styles): `npm i --no-save playwright` in the project you are working in, then a small ' +
  '`require(\'playwright\')` script. If the browser binary is missing, `npx playwright install ' +
  'chromium` once — it caches per-machine.';
...
// A card's 'default' model/effort resolves to the board-wide default from ⚙
// Settings at launch time (unset board default = let the CLI pick). An
// explicit fallback-ladder model (effModel) always wins — and the board
// default itself rides models.effective(), so a blocked default steps down
// the ladder instead of relaunching into the same wall forever.
function resolveModel(task, effModel) {
  const m = effModel || task.model;
  if (m && m !== 'default') return m;
  return models.effective(state.settings.defaultModel || '') || '';
}
function resolveEffort(task) {
  const e = task.effort;
  return (!e || e === 'default') ? (state.settings.defaultEffort || '') : e;
}

function buildArgs(task, effModel) {
  const args = ['--output-format', 'stream-json', '--verbose'];
  const model = resolveModel(task, effModel);
  ...
  // `-p -- <prompt>` goes LAST: without the `--` the CLI's own parser reads a
  // prompt that starts with '-' (a `---` divider, a `- bullet`) as an unknown
  // option and dies — and `--` ends option parsing, so every flag must already
  // be on argv before it or it lands in the prompt text instead.
  args.push('-p', '--', buildPrompt(task));
  return args;
}
...
// Most informative failure text for a non-zero exit. The CLI reports its own
// trouble on stdout (the result event) — "Not logged in · Please run /login"
// arrives that way, with stderr empty. Stamping "exited with code N" over it
// threw away the only text the authgate/cooldown/offline detectors match on,
// so a logged-out CLI failed cards one by one instead of pausing the board.
function exitError(stderr, prior, resultText, code) {
  return String((stderr || '').trim() || prior || resultText || `claude exited with code ${code}`).slice(-2000);
}
...
// Next launchable card: queued, all deps done, cards from an already-in-
// progress group preferred over a fresh group, highest priority first, then
// FIFO (state.tasks is newest-first, so walk from the end for oldest-first).
// The group-progress set is precomputed once per call — groupHasProgress was
// O(n) per comparison, making this loop O(n²) on big boards.
//
// Groups used to be a hard lane (at most one running member at a time), which
// meant an imported batch — `queue: true` launches its whole first wave at
// once, ignoring the lane — ran its opening cards in parallel and then every
// card after that one at a time, including unrelated follow-ups sharing the
// group. Dropping the exclusion lets a group use as many of the board's
// maxConcurrent slots as are free; anything that must run in sequence within
// a group already has an explicit `after:` chain (lib/deps.js), which this
// never overrode anyway.
function nextRunnable() {
  const progressGroups = new Set();
  for (const t of state.tasks) {
    if (t.group && (t.status === 'running' || t.status === 'review' || t.status === 'done')) {
      progressGroups.add(t.group);
    }
  }
  let best = null;
  for (let i = state.tasks.length - 1; i >= 0; i--) {
    ...
    const tProgress = t.group ? progressGroups.has(t.group) : false;
    const bestProgress = best.group ? progressGroups.has(best.group) : false;
...
function notifyBlocker(dep) {
  ...
  touch(dep); // save-only mutation — no type:'task' broadcast stamps this one
  save();
...
function notifyGroupCompletions() {
  ...
  for (const [group, members] of byGroup) {
    ...
    touch(last); // save-only mutation — no type:'task' broadcast stamps this one
    save();
...
function pumpQueue() {
  ...
  while (!cooldown.active() && !offline.active() && !budget.active() && !authgate.active() && slotsFree()) {
...
// Why a manual ▶ Run parked the card instead of launching it. Pressing Run and
// watching nothing happen reads as a broken button; every refusal owes the
// human a sentence.
function startBlockedReason() {
  if (cooldown.active()) return 'subscription limits are cooling down';
  if (offline.active()) return 'the connection is down';
  if (budget.active()) return 'the usage budget for this window is spent';
  return `all ${state.settings.maxConcurrent || 2} run slots are busy`;
}
...
function startTask(id) {
  ...
  if (waiting.length) {
    queueTask(task);
    const titles = waiting.map((d) => d.title);
    return { queued: true, waitingOn: titles, reason: `waits on ${titles.join(' · ')}` };
  }

  if (cooldown.active() || offline.active() || budget.active() || !slotsFree()) {
    queueTask(task);
    return {
      queued: true,
  ...
  // A pending follow-up means this card already has a session waiting for
  // its extra instructions — pressing ▶ Run here must resume it, the same as
  // pumpQueue would, instead of discarding the follow-up and starting over.
  if (task.pendingFollowUp) {
    startResume(task);
    return { started: true };
  }
  ...
  task.ctxTokens = null;
  task.liveOut = 0;
  task.prAdoptMisses = null; // a fresh run may open the PR the last one didn't
  clearTranscript(id);
  save();
  jev.shadowRoute(task); // records a would-be model pick only; never awaited
...
function followUp(id, message) {
  ...
  // Human-initiated, like the Run button — so when ONLY the auth gate is up,
  // start directly instead of parking behind the gated pump: the follow-up is
  // a probe, and its clean finish clears the gate after `claude /login`.
  if (authgate.active() && !cooldown.active() && !offline.active() && !budget.active() && slotsFree()) {
    startResume(task);
    return { started: true };
  }
  queueTask(task);
...
async function startResume(task) {
  ...
  task.error = null;
  task.permissionBlocked = null;
  // Same clear a fresh run does. exitError() falls back to resultText when a
  // run dies with no stderr and no result event, so leaving the PREVIOUS run's
  // success prose here would stamp it as this run's error — and that text goes
  // to the cooldown/offline detectors, which can freeze the whole board on
  // words like "rate limit" that merely appeared in a summary.
  task.resultText = null;
  ...
  const args = ['-r', task.sessionId, '--output-format', 'stream-json', '--verbose'];
  // Honor the card's (possibly just-changed) model/effort on resume too —
  // through the same fallback-ladder + board-default resolution as a fresh run.
  const model = resolveModel(task, models.effective(task.model));
  if (model) args.push('--model', model);
  const effort = resolveEffort(task);
  if (effort) args.push('--effort', effort);
  if (task.permissionMode) args.push('--permission-mode', task.permissionMode);
  if (task.worktree && task.cwd) args.push('--add-dir', task.cwd); // parent repo stays visible on resume
  args.push('-p', '--', message); // last, and after `--` — see buildArgs
  launch(task, args, cwd);
}
...
function launch(task, args, cwd) {
  ...
  child.on('close', (code) => {
    running.delete(id);
    teleAt.delete(id);
    lastAssistantText.delete(id);
    clearTimeout(watchdog);
    flushTranscripts(id); // no transcript tail lost past this point
    ...
    } else if (code !== 0) t.error = exitError(stderr, t.error, t.resultText, code);
    ...
    // Logged-out CLI? Trip the auth gate and requeue — the card isn't at
    // fault, and every launch would fail the same way until `claude /login`.
    // Manual runs stay allowed (they're the probe that clears the gate).
    if (!stopped && !t.permissionBlocked && t.error && authgate.detect(t.error)) {
      authgate.hit(t.error);
      appendTranscript(id, { kind: 'init', text: 'Claude CLI not logged in — card requeued; run `claude /login`, then launch any card to resume' });
      broadcast({ type: 'output', taskId: id, entry: { kind: 'init', text: 'not logged in — requeued until you `claude /login`' } });
      t.error = null;
      queueTask(t);
      return;
    }
    ...
    function settle() {
      if (!stopped) {
        if (t.error && t.permissionBlocked) {
          ...
          authgate.clear(); // a clean finish proves the CLI is logged in
        }
      }
...
// Cap on any single transcript/SSE entry. One verbose assistant message used
// to ship a multi-hundred-KB frame to every SSE client (and append the same
// to disk); 20KB keeps frames lean while preserving plenty of context. The
// separate 50KB cap on task.resultText (the card field) is unchanged.
const ENTRY_TEXT_CAP = 20 * 1024;
function capEntryText(s) {
  return s.length > ENTRY_TEXT_CAP ? s.slice(0, ENTRY_TEXT_CAP) : s;
}
  ...
  if (evt.type === 'system' && evt.subtype === 'init') {
    ...
    for (const block of evt.message.content) {
      if (block.type === 'text' && block.text.trim()) {
        const text = capEntryText(block.text);
        lastAssistantText.set(task.id, text);
        appendTranscript(task.id, { kind: 'assistant', text });
        broadcast({ type: 'output', taskId: task.id, entry: { kind: 'assistant', text } });
    ...
    if (u) {
      ...
      // Telemetry mutates the card on EVERY assistant event, but the
      // type:'task' broadcast is 2s-gated — the un-broadcast ticks need their
      // own v stamp so clients can dirty-check against what save() persists.
      touch(task);
    ...
    const resultText = capEntryText(evt.result || '');
    if (resultText && resultText !== lastAssistantText.get(task.id)) {
      entry = { kind: 'result', text: resultText };
    } else {
      // No new visible entry, but resultText/stats/error still need to reach the UI.
      save();
      broadcast({ type: 'task', task });
    }
  }
...
// True from the moment a launch is committed (startTask/startResume added the
// id to `starting`, holding its slot across the pre-launch awaits) through to
// the child process actually exiting — not just while `running` holds a live
// pid. Callers (the /run and delete routes) use this to reject a second
// launch attempt with a clean 409 instead of racing startTask/startResume's
// own internal starting-set check, which would otherwise return a confusing
// 200 "not startable" for a launch that is simply still spinning up.
...
function stopAll() {
  for (const [id, child] of running) {
    ...
    if (task) {
      ...
      touch(task); // shutdown-stamped cards get no broadcast — persist their v
    }
    child.kill('SIGTERM');
  }
}

module.exports = { exitError, startTask, stopTask, stopAll, followUp, isRunning, setOnFinish, pumpQueue, nextRunnable, notifyGroupCompletions, buildPrompt, buildArgs, handleEvent };
