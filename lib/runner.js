...
// A queued card held behind a review-status dep means a human (or Sensei)
// verdict is the critical path — bubble that up instead of letting it sit
// buried in a transcript. Notifies once per landing (dedupe key: the
// finishedAt it notified for, so a re-run that lands again renotifies).
function notifyBlocker(dep) {
  if (dep.status !== 'review' || dep.blockNotifiedFor === dep.finishedAt) return;
  const waiting = deps.dependentsOf(dep.id).filter((t) => t.status === 'queued');
  if (!waiting.length) return;
  dep.blockNotifiedFor = dep.finishedAt || 'unknown';
  save();
  const names = waiting.map((t) => `"${t.title}"`).join(', ');
  const action = dep.error
