...
async function adoptPrUrl(t) {
  ...
  const res = await run(
    'gh',
    ['pr', 'list', '--head', wt.branch, '--state', 'all', '--limit', '1', '--json', 'url', '-q', '.[0].url'],
    t.cwd,
    30_000
  );
  if (!res.ok || !res.out.startsWith('https://')) return;
  t.prUrl = res.out;
  save();
  broadcast({ type: 'task', task: t });
  log(t, 'pr', `PR watch: adopted ${res.out} — the card's PR URL was never recorded`);
