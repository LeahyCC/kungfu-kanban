...
function trackChecks(t, info) {
  ...
  if (checks.failing > 0) {
    ...
    // a fresh red later (new push) earns fresh auto-fix attempts
    if (t.ciFixes || t.ciFixGaveUp) { t.ciFixes = 0; delete t.ciFixGaveUp; save(); }
  }
  return ready;
}
  ...
  try {
    for (const t of [...state.tasks]) {
      ...
      if (info.state === 'MERGED') {
        ...
        const watching = Date.now() - Date.parse(c.firstSeenAt || 0);
...
module.exports = { sweep, applyInterval, summarizeChecks, trackChecks, maybeFixCi, backfillMergedAt };
