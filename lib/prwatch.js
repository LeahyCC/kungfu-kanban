...
// Two different automations used to ride one switch: resolving a merge
// conflict (a merge commit pushed to the branch) and fixing red CI (a fresh
// agent run on the card's own session). Wanting one without the other is
// ordinary, so they get a flag each — both default on, as before.
function autoFixEnabled() {
  return state.settings.prWatchAutoFix !== false;
}
function autoFixCiEnabled() {
  return state.settings.prWatchAutoFixCi !== false;
}
  ...
  for (const c of rollup || []) {
    ...
    if (c.__typename === 'StatusContext') {
      ...
      if (con === 'SUCCESS' || con === 'NEUTRAL' || con === 'SKIPPED') out.passing++;
        ...
        // First failing Actions check's URL — probeInfra derives the run id
        // from it. Set only when present so an empty rollup stays shape-stable.
        if (!out.failedUrl && c.detailsUrl) out.failedUrl = c.detailsUrl;
      }
    }
  }
...
function trackChecks(t, info) {
  ...
  // A verified account-level block (probeInfra) outlives count/name churn
  // while the PR stays red. "Still red" is judged by the last SETTLED state,
  // not this sweep's counts: a new CI attempt blanks the counts on its way
  // through pending, and dropping the verdict there would strand the card
  // without its badge — the re-settle is no longer ready, so nothing re-probes.
  // A settled green clears it, so a fresh red spell probes again.
  if ((checks.failing > 0 || (!settled && prevSettledFailed)) && prev && prev.infra !== undefined) {
    t.prChecks.infra = prev.infra;
    t.prChecks.infraNote = prev.infraNote;
  }
  save();
  ...
  if (checks.failing > 0) {
    ...
    t.ciInfraNotified = null; // account block over — a future one re-alerts
...
// Red checks whose jobs NEVER started (runner_id 0, zero steps) are an
// account-level GitHub Actions problem — billing / spending limit — not this
// card's code. One jobs-API probe per settled red set stamps prChecks.infra
// so the board badge says WHY the PR is red, the auto CI fixer stands down,
// and the Sensei stops burning retries on failures no push can fix.
const RUN_URL_RE = /github\.com\/([^/]+)\/([^/]+)\/actions\/runs\/(\d+)/;

function infraFromJobs(jobs) {
  const failed = (jobs || []).filter((j) => j && j.conclusion === 'failure');
  return failed.length > 0 && failed.every((j) => !j.runner_id && !(j.steps || []).length);
}

const INFRA_FALLBACK_NOTE =
  'jobs fail in seconds without a runner ever starting — check the GitHub account\'s Actions billing/spending limit';

async function probeInfra(t) {
  const c = t.prChecks;
  if (!c || !c.failing || c.pending || c.infra !== undefined) return;
  const m = RUN_URL_RE.exec(c.failedUrl || '');
  if (!m) {
    // not an Actions check (legacy status, external CI) — nothing to probe
    c.infra = false;
    touch(t);
    save();
    return;
  }
  const res = await run('gh', ['api', `repos/${m[1]}/${m[2]}/actions/runs/${m[3]}/jobs`], t.cwd, 30_000);
  let jobs;
  try {
    jobs = JSON.parse(res.out).jobs;
  } catch {}
  if (!res.ok || !Array.isArray(jobs)) return; // transient gh trouble — re-probe next sweep
  c.infra = infraFromJobs(jobs);
  if (c.infra) {
    // GitHub's check annotation carries the human-readable reason ("recent
    // account payments have failed or your spending limit needs to be
    // increased…") — quote the actual problem instead of a guess.
    const firstFailed = jobs.find((j) => j.conclusion === 'failure');
    const ann = await run('gh', ['api', `repos/${m[1]}/${m[2]}/check-runs/${firstFailed.id}/annotations`], t.cwd, 30_000);
    try {
      c.infraNote = (JSON.parse(ann.out)[0] || {}).message || null;
    } catch {}
    if (!c.infraNote) c.infraNote = INFRA_FALLBACK_NOTE;
    if (!t.ciInfraNotified) { // once per red spell, not once per CI attempt
      t.ciInfraNotified = true;
      log(t, 'error', `PR watch: the failing checks never started — a GitHub Actions account issue, not this card's code: ${c.infraNote}`);
      notify('Kungfu Kanban — CI blocked by GitHub account', `${t.title} · fix Actions billing/spending limit`, t.prUrl);
      errlog.capture('ci-failing', {
        taskId: t.id, taskTitle: t.title,
        text: `CI blocked by the GitHub account (jobs never start): ${String(c.infraNote).slice(0, 250)}`,
        detail: t.prUrl,
      });
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < watched.length; i++) {
      ...
      if (info.state === 'MERGED') {
        ...
        let fixing = false;
        const c = t.prChecks;
        if (t.status === 'review' && !conflicting && !c.wrongBase && c.failing > 0 && !c.pending) {
          // Not gated on `ready`: a re-settled identical red set is no longer
          // ready, so a probe that hit transient gh trouble would never retry
          // and the card would sit red with no explanation. probeInfra returns
          // immediately once it has a verdict, and its alert fires once per red
          // spell, so calling it on every settled-red sweep costs nothing.
          await probeInfra(t);
          // An account-level block can't be fixed by resuming the session —
          // don't spend the 2 auto-fix attempts (or a push) on it.
          if (ready && !c.infra && autoFixCiEnabled()) fixing = await maybeFixCi(t, c);
        }
        ...
        const ciUnsettled = t.status === 'review'
          && (c.pending > 0 || info.mergeable === 'UNKNOWN' || (!c.noCi && !(c.passing + c.failing + c.pending > 0)));
        // A done card approved before its PR merged has nothing left to wait
        // on but the merge itself — CI is irrelevant once it's off 'review'.
        // Left at the base watchMinutes() interval (10 min default), a queued
        // after: dependent sat blocked for however long that took, even
        // though a manual run showed it was ready the whole time. Hot-poll it
        // the same way, but only while something is actually waiting on it —
        // no point spending gh calls chasing a merge nothing depends on.
        const awaitingMerge = t.status === 'done' && !t.prMergedAt && deps.dependentsOf(t.id).length > 0;
        if (watching < HOT_POLL_MAX_MS && (ciUnsettled || awaitingMerge)) {
          hot = true;
        }
      }
    }
...
module.exports = { needsPrUrl, prBelongsTo, sweep, applyInterval, summarizeChecks, trackChecks, maybeFixCi, backfillMergedAt, probeInfra, infraFromJobs };
