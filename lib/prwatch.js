...
function trackChecks(t, info) {
  ...
  const wasReady = !!prev && (prev.noCi || (prev.pending === 0 && (prev.passing || 0) + (prev.failing || 0) > 0));
  const ready = (settled || noCi) && (!wasReady || failedChanged);
  t.prChecks = { base: info.baseRefName, ...checks, wrongBase, conflicting, noCi, firstSeenAt, key, at: new Date().toISOString() };
  // A verified account-level block (probeInfra) outlives count/name churn
  // while the PR stays red; any state without failures drops it, so a fresh
  // red spell re-probes rather than trusting a stale verdict.
  if (checks.failing > 0 && prev && prev.infra !== undefined) {
  ...
  try {
    ...
    for (let i = 0; i < watched.length; i++) {
      ...
      if (info.state === 'MERGED') {
        ...
        let fixing = false;
        const c = t.prChecks;
        if (ready && t.status === 'review' && !conflicting && !c.wrongBase && c.failing > 0) {
          await probeInfra(t);
          // An account-level block can't be fixed by resuming the session —
          // don't spend the 2 auto-fix attempts (or a push) on it.
          if (!c.infra && autoFixEnabled()) fixing = await maybeFixCi(t, c);
        }
