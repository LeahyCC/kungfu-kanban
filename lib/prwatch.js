...
//                 conflicting) so the board badges red PRs and the Sensei won't
//                 approve them; transitions (new failure, recovery) log + notify
//                 once. When a review card's checks settle (nothing pending),
//                 the sweep re-invokes the Sensei so the verdict actually lands —
//                 the finish-time review runs before CI has reported anything.
const fs = require('fs');
...
// decision-ready (checks settled, or the settled outcome changed) so the
// sweep can hand the card back to the Sensei for its verdict.
function trackChecks(t, info) {
  ...
  const key = [info.baseRefName, checks.failing, checks.pending, checks.passing, checks.failed.join(','), wrongBase, conflicting].join('|');
  ...
  const settled = checks.pending === 0 && checks.passing + checks.failing > 0;
  const wasReady = !!prev && prev.pending === 0 && (prev.passing || 0) + (prev.failing || 0) > 0;
  const ready = settled && (!wasReady || failedChanged);
  t.prChecks = { base: info.baseRefName, ...checks, wrongBase, conflicting, key, at: new Date().toISOString() };
  save();
  ...
  try {
    for (const t of [...state.tasks]) {
      ...
      if (info.state === 'MERGED') {
        ...
        // reported anything, so once the checks settle hand the card back for
        // the actual merge/reject verdict. Same gate as the finish trigger.
        if (ready && t.status === 'review') {
...
module.exports = { sweep, applyInterval, summarizeChecks, trackChecks, backfillMergedAt };
