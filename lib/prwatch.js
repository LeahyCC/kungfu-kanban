...
async function adoptPrUrl(t) {
  ...
  // Branch NAMES are the agent's choice and can repeat across cards, so a hit
  // only belongs to THIS card if it was opened after the card started running.
  // Without that check a stale merged PR from whoever used the name first would
  // mark unshipped work done and release its dependents.
  if (pr && t.startedAt && Date.parse(pr.createdAt) < Date.parse(t.startedAt)) pr = null;

  if (!pr || !String(pr.url).startsWith('https://')) {
...
module.exports = { needsPrUrl, sweep, applyInterval, summarizeChecks, trackChecks, maybeFixCi, backfillMergedAt };
