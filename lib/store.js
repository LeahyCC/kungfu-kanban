...
function appendTranscript(id, entry) {
  // Stamped here rather than at each of the ~18 call sites — every transcript
  // line goes through this function, so one line gives the whole log a clock.
  // Callers that broadcast a separate literal to SSE leave it unstamped; the
  // client stamps those on arrival (live entries are, by definition, now).
...
// Moves "done" cards out of state.tasks and into data/archive.jsonl
// (append-only), dropping their transcript files. The daily sweep takes only
// cards older than settings.archiveDays; `{ all: true }` takes every done card
// regardless of age, which is what "clear the Done column" asks for.
function sweepArchive({ all = false } = {}) {
  const days = state.settings.archiveDays;
  if (!all && (!days || days <= 0)) return [];
  ...
  for (const t of state.tasks) {
    ...
    const aged = Number.isFinite(ts) && ts < cutoff;
    if (t.status === 'done' && (all || aged)) {
      archived.push(t);
    } else {
      keep.push(t);
    }
...
// Read-only view over the append-only archive; corrupt lines are skipped.
// Returns append order (oldest → newest); callers reverse for display.
