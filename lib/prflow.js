...
async function openPr(taskId, broadcast) {
  ...
  // Pick a PR base that actually exists on origin: the checkout's current
  // branch if it's pushed, else the remote default. (A checkout sitting on a
  // local-only feature branch must not become the base — GitHub rejects it.)
  const cur = await run('git', ['branch', '--show-current'], task.cwd);
  let baseBranch = cur.ok && cur.out ? cur.out : 'main';
  const onOrigin = await run('git', ['rev-parse', '--verify', '--quiet', `origin/${baseBranch}`], task.cwd);
  if (!onOrigin.ok) {
    ...
    log('pr', `PR flow: checkout is on a local-only branch — basing the PR on ${baseBranch}`);
  }
  ...
  const createArgs = ['pr', 'create', '--head', wt.branch, '--base', baseBranch, '--title', task.title.slice(0, 200), '--body', body];
  let pr = await run('gh', createArgs, wt.path, 120_000);
  if (!pr.ok && /sha can't be blank|could not resolve|not found/i.test(pr.err)) {
    ...
    pr = await run('gh', createArgs, wt.path, 120_000);
  }
