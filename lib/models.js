...
function fallbackFor(task, err) {
  const short = LADDER.find((m) => (task.modelUsed || '').includes(m));
  const failed = short || (LADDER.includes(task.model) ? task.model : null);
...
module.exports = { detect, block, blocks, effective, fallbackFor, LADDER };
