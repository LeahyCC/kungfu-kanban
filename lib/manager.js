    ...
    actions: {
      ...
      items: {
        ...
        properties: {
          type: {
            type: 'string',
            enum: ['create_task', 'update_task', 'run_task', 'approve_task', 'reject_task', 'requeue_task', 'retarget_pr', 'merge_pr', 'resolve_error', 'note'],
          },
          taskId: { type: 'string' },
          title: { type: 'string' },
          prompt: { type: 'string' },
...
function handleDecision(decision, trigger, isChat) {
  ...
  for (const action of decision.actions || []) {
    ...
    const needsApproval =
      c.autonomy === 'suggest' ||
      (c.autonomy === 'semi' && (action.type === 'approve_task' || action.type === 'reject_task' || action.type === 'merge_pr')) ||
      guard;
...
module.exports = { config, invoke, chat, resolveSuggestion, publicState, applyInterval, clearChat, clearLog, executeAction };
