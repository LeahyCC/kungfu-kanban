...
function buildPrompt(trigger, userMessage) {
  ...
  const lines = [
    ...
    'If a card\'s error says it was blocked on permission, a plain retry will hit the same wall — the permission mode is the blocker. Raise it within your ceiling via update_task if the work warrants it, otherwise leave a note for the human (do not burn a retry).',
...
function executeAction(a) {
  switch (a.type) {
    ...
    case 'update_task': {
      ...
      for (const f of ['title', 'prompt', 'cwd', 'model', 'effort', 'skills', 'skillsAuto', 'worktree', 'openPr', 'priority', 'acceptanceCriteria']) {
