...
const { state, save, saveSettings, getTask, writeJsonAtomic, DATA_DIR } = require('./store');
...
// At 'semi' these four are the verdicts held back for the human's ✓ — they end
// or restart work rather than start it. autoActions releases them one at a
// time, so "merge green PRs yourself but ask before approving" is expressible;
// all-false is the classic 'semi' the ladder has always described.
const SEMI_HELD = ['approve_task', 'reject_task', 'followup_task', 'merge_pr'];
...
const DEFAULT_CONFIG = {
  ...
  autoActions: { approve_task: false, reject_task: false, followup_task: false, merge_pr: false },
...
function config() {
  const cur = state.settings.manager || {};
  const merged = { ...DEFAULT_CONFIG, ...cur };
  merged.triggers = { ...DEFAULT_CONFIG.triggers, ...(cur.triggers || {}) };
  merged.autoActions = { ...DEFAULT_CONFIG.autoActions, ...(cur.autoActions || {}) };
  state.settings.manager = merged;
  // config() runs on every read (GET /api/manager polls included), so persist
  // only when normalization actually filled a gap — an unconditional
  // saveSettings() here would rewrite settings.json on every poll.
  if (JSON.stringify(cur) !== JSON.stringify(merged)) saveSettings();
  return merged;
}
...
function log(kind, text, extra = {}) {
  ...
  // Keep the readLog cache coherent without a re-read: push the new entry.
  // trimLog only ever drops head lines older than any readLog(n) window, so
  // a cache that grows past a trim still slices the same tail.
  if (logCache) logCache.push(entry);
  trimLog();
...
// Parsed-log cache: GET /api/manager used to re-read and re-parse the whole
// jsonl (up to 200KB) on every poll — and the client polls on every task SSE
// event. The file is append-only through log() (which pushes into the cache)
// and only ever rewritten by trimLog (drops old head lines — the tail any
// readLog(n) returns is untouched) or clearLog (resets the cache).
let logCache = null; // parsed entries, oldest → newest; null = not read yet
function readLog(n = 40) {
  if (!logCache) {
    try {
      logCache = fs
        .readFileSync(MGR_LOG, 'utf8')
        .split('\n')
        .filter(Boolean)
        .map((l) => JSON.parse(l));
    } catch {
...
async function snapshot() {
  ...
  const rows = state.tasks.map((t) => ({
    ...
    prChecks: t.prChecks
      ? { base: t.prChecks.base, passing: t.prChecks.passing, failing: t.prChecks.failing, pending: t.prChecks.pending, failed: t.prChecks.failed, wrongBase: t.prChecks.wrongBase, conflicting: t.prChecks.conflicting, noCi: t.prChecks.noCi, ciFixAttempts: t.ciFixes || undefined, infra: t.prChecks.infra || undefined, infraNote: t.prChecks.infra ? t.prChecks.infraNote : undefined }
      : undefined,
    deps: (t.deps || []).length
      ? t.deps.map((id) => {
          const d = getTask(id);
...
async function buildPrompt(trigger, userMessage) {
  ...
  const lines = [
    ...
    'UI verification: every card prompt tells agents to prove UI changes in a real headless browser (npx playwright) and report what they verified. When a review card\'s diff touches frontend files (HTML/CSS/client JS) and its result reports no browser verification, do not approve or merge on code review alone — followup_task it asking for the browser check and its evidence (no retry burned).',
    ...
    'PR checks: a review card with an open PR carries prChecks {base, passing, failing, pending, failed[], wrongBase, conflicting, noCi, ciFixAttempts}. conflicting: true means the PR has merge conflicts with its base — never merge_pr it; the watcher auto-spawns a conflict-fix run (when enabled), so wait for it to clear, and if it persists the human resolves by hand. NEVER merge_pr or approve_task while failing > 0, wrongBase is true, or pending > 0 (CI still running — wait, or note it and check back later). If prChecks is absent entirely, or present with passing+failing+pending all zero, no CI has actually reported on the PR yet — treat that as unknown, not green: do not merge_pr, wait for checks to appear. The one exception is noCi: true — the repo has no CI at all (nothing reported within the grace window), so judge the diff on its own merits and merge_pr when it satisfies the criteria. Failing checks on a clean PR get an automatic fix first: the watcher resumes the card\'s own session with the failing logs (ciFixAttempts counts these, max 2, no retry burned) — while a red card\'s ciFixAttempts < 2 the auto-fix is still in play, so wait rather than reject. Once it\'s exhausted (or for failures a session resume can\'t fix), reject_task naming the failed checks (the retry reuses the same worktree; its push updates the same PR) — but if the failures are CI infrastructure (billing, runners never starting), no code change can fix them: leave a note for the human instead of burning retries. When prChecks carries infra: true the watcher has VERIFIED that case (the failed jobs died with no runner assigned; infraNote quotes GitHub\'s reason) — never reject_task, followup_task, or requeue_task over those failures; the only fix is the human repairing the account. A failing branch-guard check (e.g. "source-must-be-staging") or wrongBase means the PR targets the wrong base branch: fix it yourself with retarget_pr (taskId + prBaseBranch) — it moves the open PR via gh pr edit --base and updates the card; a plain retry cannot move an existing PR. Once failing === 0 and pending === 0 and the base is correct, merge_pr ships it.',
...
async function invoke(trigger, userMessage = null) {
  const c = config();
  if (!c.enabled) return;
  // The Sensei's own run burns tokens too — all three guards pause it. The
  // auth gate exempts USER-INITIATED chat: a chat run is a free probe (it
  // fails fast while logged out, and its success clears the gate).
  const paused = require('./cooldown').active()
    ? '⏳ Subscription limits are cooling down — I\'ll be back when they reset.'
    : require('./budget').active()
      ? '⛽ The 5-hour usage budget is spent — I\'ll be back when the window slides.'
      : (!userMessage && require('./authgate').active())
        ? '🔐 The Claude CLI is logged out — run `claude /login` in a terminal, then launch any card and I\'ll resume.'
        : null;
  if (paused) {
    if (userMessage) {
      mgr.chat.push({ role: 'manager', text: paused, ts: new Date().toISOString() });
      saveMgr();
  ...
  // Idle gate for the polling trigger — event-driven triggers (new card,
  // finish, PR settled, chat) always run. Checked after the busy gate so a
  // queued pending interval re-evaluates the board when it finally fires.
  if (trigger === INTERVAL_TRIGGER && boardIdle()) {
    if (!idleSkipLogged) {
      idleSkipLogged = true;
      log('info', 'interval check skipped — nothing queued/running/in review, no open errors or suggestions');
    }
    return;
  }
  busy = true;
  ...
  args.push('-p', '--', prompt); // last: `--` ends option parsing — see runner.buildArgs
...
// trigger.
// Trip the gate this failure belongs to (subscription limit, logged-out CLI)
// and report it as one deduped pause rather than an error per trigger.
// Returns true when it handled the message.
function pauseFor(msg, status429 = false) {
  if (status429 || require('./cooldown').detect(msg)) {
    require('./cooldown').hit(msg);
    log('note', `Paused — ${msg.replace(/\s+/g, ' ').slice(0, 160)}`);
    return true;
  }
  if (require('./authgate').detect(msg)) {
    require('./authgate').hit(msg);
    log('note', 'Paused — Claude CLI is not logged in. Run `claude /login`, then launch any card to resume.');
    return true;
  }
  return false;
}
...
function finishInvocation(out, err, trigger, isChat) {
  ...
  if (wrapper.is_error) {
    ...
    // stderr is the only place a launch-level failure explains itself; dropping
    // it left "Manager run errored: …" unactionable in the log.
    if (!pauseFor(msg, wrapper.api_error_status === 429)) {
      const tail = (err || '').trim();
      log('error', `Manager run errored: ${msg.slice(0, 300)}${tail ? ` · stderr: ${tail.slice(0, 200)}` : ''}`);
    }
    return;
  }
  let decision;
  try {
    ...
    // A "successful" run whose result isn't the decision JSON is the CLI
    // reporting its own trouble in prose ("Not logged in · Please run /login",
    // a session-limit notice). It needs the same gate as an is_error run —
    // otherwise every interval logs one more unactionable entry forever.
    const msg = String(wrapper.result || '');
    if (pauseFor(msg)) return;
    log('error', `Manager output unparsable (${e.message}): ${(err || msg || out).slice(0, 300)}`);
    return;
  }
  require('./authgate').clear(); // a parsed decision IS a logged-in run
...
function handleDecision(decision, trigger, isChat) {
  ...
  for (const action of decision.actions || []) {
    ...
    const needsApproval = heldForApproval(action.type, c) || guard;
...
// Does the autonomy ladder hold this action for the human's ✓? Guardrails
// (launch cap, retry limit) are a separate, additive gate — see the caller.
function heldForApproval(type, c = config()) {
  if (c.autonomy === 'suggest') return true;
  if (c.autonomy !== 'semi') return false; // 'auto'
  return SEMI_HELD.includes(type) && !c.autoActions[type];
}
...
function executeAction(a, batch = []) {
  switch (a.type) {
    ...
    case 'requeue_task': {
      const t = getTask(a.taskId);
      if (!t) return { error: 'task not found' };
      if (t.status === 'running' || t.status === 'stopping') return { ok: true, note: 'card is running — requeue skipped, can wait' };
    ...
    case 'merge_pr': {
      ...
      if ((checks.passing || 0) + (checks.failing || 0) + (checks.pending || 0) === 0 && !checks.noCi) {
      ...
      if (checks.conflicting) return { ok: true, note: 'PR has merge conflicts with its base — not merging' };
...
const INTERVAL_TRIGGER = 'scheduled interval check';
function applyInterval() {
  clearInterval(intervalTimer);
  const min = config().triggers.intervalMin;
  if (min > 0) intervalTimer = setInterval(() => invoke(INTERVAL_TRIGGER), min * 60_000);
}

// The interval is a polling trigger: it only earns a CLI run when there's
// something to decide. Without this gate an idle board got a "nothing to
// do — standing by" Sensei run every intervalMin forever (and, when the CLI
// is logged out, an error entry each time).
const ACTIVE_STATUSES = new Set(['queued', 'running', 'stopping', 'review']);
function boardIdle() {
  if (state.tasks.some((t) => ACTIVE_STATUSES.has(t.status))) return false;
  pruneSuggestions();
  if (mgr.suggestions.length) return false;
  if (errlog.openCount() > 0) return false;
  return true;
}
...
module.exports = { pauseFor, config, invoke, chat, resolveSuggestion, publicState, applyInterval, clearChat, clearLog, executeAction, heldForApproval, suggestionLive, pruneSuggestions, stopCurrent, log, boardIdle, SEMI_HELD, INTERVAL_TRIGGER };
