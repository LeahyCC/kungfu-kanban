...
function finishInvocation(out, err, trigger, isChat) {
  ...
  if (wrapper.is_error) {
    ...
    } else if (require('./authgate').detect(msg)) {
      // one deduped ⚠ entry + paused triggers, not an error line per interval
  ...
  require('./authgate').clear(); // a run that came back with a result IS logged in
  let decision;
  try {
...
module.exports = { config, invoke, chat, resolveSuggestion, publicState, applyInterval, clearChat, clearLog, executeAction, suggestionLive, pruneSuggestions, stopCurrent, log, boardIdle, INTERVAL_TRIGGER };
