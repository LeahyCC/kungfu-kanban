...
const { state, save, getTask, readTranscript, writeJsonAtomic } = require('./store');
  ...
  switch (a.type) {
    ...
    case 'retarget_pr': {
      ...
      execFile('gh', ['pr', 'edit', t.prUrl, '--base', base], { cwd: t.cwd, timeout: 60_000 }, (err, _out, stderr) => {
        if (err) {
          log('error', `retarget ${short(a.taskId)} → ${base} failed: ${(stderr || err.message || '').trim().slice(0, 200)}`, { action: a });
        } else {
    ...
    case 'resolve_error': {
      ...
      return e ? { ok: true } : { error: 'error entry not found (already resolved?)' };
    }
    case 'reject_task': {
      const t = getTask(a.taskId);
      if (!t) return { error: 'task not found' };
