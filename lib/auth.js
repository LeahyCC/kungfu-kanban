...
// Token cache: getToken() runs on every request (~15 per page load), and the
// token only changes when someone rewrites the file — so stat the file and
// re-read only when the mtime moves. A missing/unreadable file is cached as
// mtime -1 → null. The per-request gate semantics are unchanged: every request
// still calls getToken() and compares with timingSafeEqual.
let cachedMtime = null; // null = nothing cached yet; -1 = file absent
...
function getToken() {
  ...
  let mtime = -1;
  try {
    mtime = fs.statSync(TOKEN_FILE).mtimeMs;
  } catch {}
  if (mtime !== cachedMtime) {
    ...
    if (mtime !== -1) {
      try {
        cachedToken = fs.readFileSync(TOKEN_FILE, 'utf8').trim() || null;
      } catch {
        cachedToken = null;
      }
    }
...
module.exports = { install, getToken, TOKEN_FILE, tokenMatches, parseCookies };
