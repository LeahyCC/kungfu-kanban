...
function hit(msg) {
  ...
  require('./errlog').capture('limit', {
    text: `subscription limit hit — auto flow paused until ${new Date(t).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}`,
...
function clear() {
  ...
  require('./errlog').resolveKind('limit');
  arm();
}
...
function arm() {
  ...
  timer = setTimeout(() => {
    ...
    require('./errlog').resolveKind('limit'); // the block lifted itself
    require('./runner').pumpQueue(); // lazy: avoids a require cycle
  }, ms + 2000);
}

module.exports = { setBroadcaster, active, until, detect, parseReset, hit, clear };
