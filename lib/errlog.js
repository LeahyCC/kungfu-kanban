...
const { writeJsonAtomic, DATA_DIR } = require('./store');
const { broadcast } = require('./bus');

const FILE = path.join(DATA_DIR, 'errors.json');
const MAX_ENTRIES = 400; // history cap; resolved entries also age out (14 days)
const KINDS = ['permission', 'wrong-base', 'ci-failing', 'pr-flow', 'pr-conflict', 'run-failed', 'launch-failed', 'sensei', 'limit', 'import', 'offline', 'budget', 'auth'];
...
module.exports = { capture, resolve, resolveAll, resolveTask, resolveKind, list, forPrompt, openCount };
