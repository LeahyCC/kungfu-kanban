...
const { state, save, DATA_DIR } = require('./store');
...
// A relative path (./, ../, ~/) or a POSIX absolute path (leading /) is
// already covered by the [./~] class below. Windows adds two more shapes:
// a drive letter ("C:\..." or "C:/...") and a UNC share ("\\server\share"
// or "//server/share").
const WIN_OR_UNC_CWD = /^(?:[A-Za-z]:[\\/]|[\\/]{2}[^\\/])/;

// Accepts one "key: value" line; returns a partial card field or null. `warn`
// (optional) is called with the raw value when a cwd-shaped key fails
// validation, so the caller can report it instead of the value silently
// evaporating into "use the board default".
function parseField(key, value, warn) {
  const k = key.toLowerCase().replace(/[\s_-]/g, '');
  const v = value.trim();
  switch (k) {
    case 'cwd': case 'dir': case 'directory': case 'repo': {
      if (/^[./~]/.test(v) || WIN_OR_UNC_CWD.test(v)) {
        // The rest of the app treats cwd as a POSIX-style string (path.join,
        // git args, etc.) — normalise Windows backslashes once, here, so
        // nothing downstream has to special-case them.
        return { cwd: v.replace(/\\/g, '/') };
      }
      if (warn) warn(v);
      return null;
    }
    ...
    case 'group': case 'batch':
      return v ? { group: v.slice(0, 60) } : null;
    ...
    // Frontmatter flag: launch (or park behind deps) every card in the file on import.
    case 'queue': case 'autoqueue':
      return { queue: truthy(v) };
    default:
      return null;
  }
}
...
// Frontmatter (--- ... ---) → file-wide defaults + remaining text. `warnings`
// collects { card: null, value } for a cwd-shaped line that failed validation
// (null card = applies to every card in the file, not one heading).
function splitFrontmatter(text, warnings) {
  ...
  for (const line of m[1].split('\n')) {
    ...
    if (kv) mergeField(defaults, parseField(kv[1], kv[2], (v) => warnings.push({ card: null, value: v })));
  }
...
function parseSection(heading, lines, warnings) {
  const fields = {};
  let i = 0;
  ...
  for (; i < lines.length; i++) {
    ...
    if (!line) continue;
    const kv = line.match(FIELD_LINE);
    const parsed = kv && parseField(kv[1], kv[2], (v) => warnings.push({ card: heading.trim(), value: v }));
    if (!parsed) break;
...
// Pure: markdown text → array of card specs. `defaultGroup` (e.g. derived from
// an inbox filename) fills in group for any card that didn't set one itself.
function parseMarkdown(text, defaultGroup) {
  // Collected here (not inside parseField/parseSection) so parseMarkdown stays
  // pure — no logging side effect just from parsing/previewing text. The
  // caller that actually creates cards (createCards) decides what to do with
  // these; attached to the returned array rather than changing its shape, so
  // every existing `const cards = parseMarkdown(...)` call keeps working.
  const warnings = [];
  ...
  if (defaultGroup && !defaults.group) defaults.group = defaultGroup;
  ...
  if (parts.length > 1) {
    ...
    cards = parts.slice(1).map((part) => {
      ...
      const section = parseSection(lines[0], lines.slice(1), warnings);
  ...
  // Only attached when non-empty: parseMarkdown's callers (and its tests)
  // treat the return value as a plain array of cards otherwise.
  if (warnings.length) cards.cwdWarnings = warnings;
  return cards;
}
...
function createCards(specs) {
  ...
  // A cwd override that failed validation (see parseField) never silently
  // vanishes — it lands here as an import-tracker entry naming the card, so
  // "why did this run against the board default instead of my repo" has an
  // answer instead of quiet guessing.
  for (const w of specs.cwdWarnings || []) {
    const hit = w.card && created.find((t) => t.title === w.card);
    const affects = w.card ? `card "${w.card}"` : `every card in this import (${created.map((t) => t.title).join(', ') || 'none created'})`;
    require('./errlog').capture('import', {
      taskId: hit ? hit.id : null,
      taskTitle: hit ? hit.title : null,
      text: `import: cwd "${w.value}" on ${affects} is not a usable path (expected ./, ~/, an absolute path, a Windows path like C:\\path, or a UNC share like \\\\server\\share) — using the board default cwd instead`,
    });
  }
  if (created.length) save();
  // Second pass, oldest-first: deps resolve against earlier cards in the
  // batch, so only start queuing once every card (and its deps) exists.
  // startTask parks a card with unmet deps in Queued — same as a human
  // pressing run on each; it never re-triggers triage.
  specs.forEach((c, i) => {
    if (c.queue) require('./runner').startTask(created[i].id);
  });
  return created;
}

function importMarkdown(text, defaultGroup) {
  return createCards(parseMarkdown(text, defaultGroup));
}

// "app-quality-audit-20260719-2057.md" -> "App quality audit" — strips the
// extension and a trailing -YYYYMMDD-HHMM stamp, dashes become spaces.
function labelFromFilename(name) {
  const base = name.replace(/\.md$/i, '').replace(/-\d{8}-\d{4}$/, '').replace(/-/g, ' ');
  return base.charAt(0).toUpperCase() + base.slice(1);
}

// Imported files pile up forever otherwise — prune anything past the archive window.
function sweepImported(maxAgeDays = 30) {
  const cutoff = Date.now() - maxAgeDays * 24 * 60 * 60 * 1000;
  for (const f of fs.readdirSync(IMPORTED)) {
    const file = path.join(IMPORTED, f);
...
// Archives `file` into IMPORTED under `destName`. rename first (fast, same
// device); falls back to copy+unlink (cross-device); errlog.captures if both fail
// so a stuck file doesn't just silently re-import on every boot.
function archive(file, destName) {
  const dest = path.join(IMPORTED, destName);
  try {
  ...
  try {
    ...
    // The source is gone: a racing scan already archived it. Nothing is
    // stranded and nothing will re-import, so the "could not archive" entry
    // would be a permanently-open error about a file that imported fine.
    if (!fs.existsSync(file)) return;
    require('./errlog').capture('import', {
      text: `inbox: could not archive ${path.basename(file)} — will re-import on next scan: ${e.message}`,
    });
  }
}
...
function processFile(name) {
  ...
  const created = importMarkdown(text, labelFromFilename(name));
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  if (created.length === 0) {
    // Leave it in the inbox, renamed so it's visibly flagged instead of
    // vanishing like a successful import — the skill teaches "file gone =
    // imported", so a silently archived zero-card file reads as success.
    const rejected = path.join(INBOX, name.replace(/\.md$/i, '') + '.rejected.md');
    try {
      fs.renameSync(file, rejected);
    } catch {}
    require('./errlog').capture('import', {
      text: `inbox: ${name} produced no cards (no headings or checklist items) — renamed to ${path.basename(rejected)}`,
    });
    console.log(`inbox: 0 cards from ${name}, left as ${path.basename(rejected)}`);
    return;
  }
  archive(file, `${stamp}-${name}`);
  console.log(`inbox: imported ${created.length} card(s) from ${name}`);
  onImported(created, `inbox file ${name}`);
}

// Debounce (500ms) settles bursts within one write, but a slow writer (scp,
// network mount) can pause mid-write longer than that — reading here would
// import a truncated file. Stat, wait, stat again; a mismatch means the
// writer is still going, so reschedule instead of importing.
function trySettledImport(name) {
  const file = path.join(INBOX, name);
  let before;
  try {
...
function watchInbox(callback) {
  ...
  for (const f of fs.readdirSync(INBOX)) {
    if (f.endsWith('.md') && !f.endsWith('.rejected.md')) processFile(f);
  }
  fs.watch(INBOX, (event, name) => {
    if (!name || !name.endsWith('.md') || name.endsWith('.rejected.md')) return;
...
queue: true               (auto-launch every card on import; chained cards park in Queued)
---
...
function execDraft(args, cwd, timeoutMs) {
  ...
  const promise = new Promise((resolve, reject) => {
    child = execFile('claude', args, { env, cwd, timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024 }, (err, stdout, stderr) => {
      if (err) {
        // A non-zero exit still usually carries the real reason on stdout as
        // JSON (usage limits, auth, model errors). Reporting err.message alone
        // echoed the whole prompt back at the user and hid the cause.
        let why = '';
        try {
          const out = JSON.parse(stdout);
          why = String(out.result || out.error || '').trim();
        } catch {}
        return reject(new Error((why || stderr || err.message || 'draft failed').replace(/\s+/g, ' ').slice(0, 300)));
      }
      try {
        const out = JSON.parse(stdout);
        let md = out.result || '';
...
function draft(request, ctx = {}) {
  ...
  const prompt = [
    ...
    '- Set "queue: true" in the frontmatter whenever the file declares after:/sequential: chains — otherwise the cards land in Backlog and nothing launches them.',
    explore
  ...
  const args = ['--output-format', 'json', '--model', model];
...
// Sharpen ONE prompt in place. Deliberately not a drafting call: it returns a
// better version of the user's own request — no cards, no board side effects —
// so the composer can offer "improve this" without committing to anything.
function improvePrompt(text, ctx = {}) {
  const prompt = [
    'You rewrite task prompts for a coding agent that will execute them with NO other context.',
    'Rewrite the request below so it is specific, self-contained, and verifiable:',
    '- keep the original intent and scope — do not invent new features or expand the ask',
    '- state the concrete outcome, the files or areas involved when they are implied, and any constraints',
    '- end with a short "Acceptance:" line naming how the agent knows it is done',
    '- if the request is already sharp, return it nearly unchanged',
    '- plain prose and short bullets only: no preamble, no headings, no code fences',
    ctx.repoPath ? `\nThe work targets the repo at ${ctx.repoPath}.` : '',
    '',
    `Request:\n${text}`,
    '',
  ...
  const model = require('./models').effective('sonnet');
  // no tools: this is a writing pass over the text, not a repo exploration
  return execDraft(['--output-format', 'json', '--model', model, '--tools', '', '-p', '--', prompt], undefined, 120_000);
}
...
module.exports = { parseMarkdown, importMarkdown, watchInbox, draft, refine, improvePrompt, resolveDep, labelFromFilename, INBOX, sweepImported };
